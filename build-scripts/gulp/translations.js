    ...
    if (value instanceof Object) {
      ...
      output[key] = value.replace(re_key_reference, (_match, lokalise_key) => {
        const replace = lokalise_key.split("::").reduce((tr, k) => {
          if (!tr) {
            throw Error(`Invalid key placeholder ${lokalise_key} in ${file.path}`);
          }
          return tr[k];
        }, original);
...
gulp.task("build-flattened-translations", () =>
  ...
  gulp
    .src(
      TRANSLATION_FRAGMENTS.map((fragment) => workDir + "/" + fragment + "/*.json").concat(
        coreDir + "/*.json"
      ),
      { base: workDir }
    )
...
gulp.task("build-translation-fingerprints", () => {
  ...
  if (env.isProdBuild()) {
    mapFiles(outDir, ".json", (filename) => {
      ...
      fs.renameSync(
        filename,
        `${parsed.dir}/${parsed.name}-${fingerprints[parsed.name].hash}${parsed.ext}`
      );
    });
  }
