...
export const getGroupTelegrams = (hass: HomeAssistant): Promise<Record<string, TelegramDict>> =>
  hass.callWS({
 ...
 * Entity store calls.
 */
export const getSchema = (hass: HomeAssistant, platform: string): Promise<SelectorSchema[]> =>
  hass.callWS({
    type: "knx/get_schema",
    platform,
  });
...
export const getEntitiesByGroup = (
  hass: HomeAssistant,
): Promise<Record<string, KNXEntityIdentifier[]>> =>
  hass.callWS({
